...
def main():
    ...
    for iteration in range(args.start_iteration, args.max_iterations + 1):
        ...
        if test_and_show:
            ...
            test(model_wrapper, iteration, trn_dataset, args.batch_size, args.print_trans, args.show_trans,
                 args.show_dir, args.pad_to_max_width, args.arabic)
...
def test(training, iteration, dataset, batch_size, print_trans, show_trans, show_dir, pad_to_max_width=False, arabic=False):
    ...
    with torch.no_grad():
        for it_count, batch in enumerate(batch_generator, 1):
            ...
            err, count = compute_character_error(labels, decoded, weights)
            total_nb_errors += err
            total_ref_len += count
            total_line_length += data.shape[0] * data.shape[2] / data.shape[1]

            if it_count == 1:
                if len(np.shape(labels)) != len(np.shape(decoded)) or np.shape(labels)[0] != np.shape(decoded)[0]:
                    labels = list(itertools.chain(*labels))
                show_batch_data = data
                show_batch_occurences = occurences
    ...
    if show_trans:
        show_transcriptions(show_batch_data, show_batch_occurences, iteration, dataset.transcriptions_file, train, show_dir, show_batch_transcriptions, show_batch_ground_truth, arabic)
...
def compute_character_error(reference, decoded, weights):
    err = 0
    count = 0

    if len(np.shape(reference)) != len(np.shape(decoded)) or np.shape(reference)[0] != np.shape(decoded)[0]:
        weights_ = []
        counter = 0
        for item in reference:
            batch_item = []
            for _ in range(len(item)):
                batch_item.append(weights[counter])
                counter += 1

            if batch_item != []:
                batch_item = softmax(batch_item)
            weights_.append(batch_item)

        weights = weights_
        weights = list(itertools.chain(*weights))

    if len(np.shape(reference)) != len(np.shape(decoded)) or np.shape(reference)[0] != np.shape(decoded)[0]:
        reference = list(itertools.chain(*reference))

    for i, (r, d) in enumerate(zip(reference, decoded)):
        err += Levenshtein.distance(''.join([chr(x) for x in r]), ''.join([chr(x) for x in d])) * weights[i]
        count += r.shape[0]
...
def init_datasets(max_buffer_size, trn_data, tst_data, images_path, lmdb_path, chars, batch_size, max_line_width,
    ...
    if trn_data is not None:
        trn_dataset = TextDatasetRandom(max_buffer_size, data_format="PyTorch", transcriptions_file=trn_data,
                                        ...
                                        max_width=max_line_width, verbose=True, transformer=transformer, weights=True)
    max_queue_size = 400
    tst_datasets = None
...
def show_transcriptions(data, occurences, iteration, dataset_name, train, show_dir, transcriptions=None, ground_truths=None, arabic=False, batch_id=None):
    images = []
    data = np.transpose(data, (0, 2, 3, 1))
    for i in range(data.shape[0]):
        for _ in range(int(occurences[i])):
